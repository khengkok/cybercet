...
def prov(num_instances, out_csvfile):
    ...
    win10appsec_ami_id = get_ami_id(win10appsec_ami) 
    ...
    instanceIdList = create_instances(win10appsec_ami_id, 
                                      subnet_secgrps_tuples, 
                                      num_instances, 
                                      auto_assign_public_ip=True,
                                      size='t2.medium')

    tag_instances(instanceIdList, 'win10appsec')                                  
    win10appsec_infos = get_instances_info(instanceIdList)
    print(win10appsec_infos)


    # format csv
    info_dict_list = []
    
    for index, win_info in enumerate(win10appsec_infos):
        info_dict = {}
        info_dict['win10appsec_instance_id'] = win_info['id']
        info_dict['win10appsec_public_ip'] = win_info['public_ip']
        name = 'win10appsec-' + str(index+1)
        tag_instance(info_dict['win10appsec_instance_id'], name)
        for intf in win_info['nets']:
            keyname = 'win10appsec_priv_ip#' + str(intf['intf_index'])
...
def deprov(csvfile):
    with open(csvfile, mode='r') as csv_file:
        ...
        for row in csv_reader:
            instance_list.append(row['win10appsec_instance_id'])
