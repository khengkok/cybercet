...
# Configuration:
# Linux in a single public subnet
...
splunk_ami = 'splunk_sc2021'

def prov(num_instances, out_csvfile):

    # win10 (forensic) image 
    splunk_ami_id = get_ami_id(splunk_ami) 
    secgrpnames_public = ['splunk_ssh_web']  # public 
    ...
    instanceIdList = create_instances(splunk_ami_id, 
    ...
    tag_instances(instanceIdList, 'splunk', 'splunkchallenge')                                  
    splunk_infos = get_instances_info(instanceIdList)
    print(splunk_infos)
    ...
    for index, win_info in enumerate(splunk_infos):
        info_dict = {}
        info_dict['splunk_instance_id'] = win_info['id']
        info_dict['splunk_public_ip'] = win_info['public_ip']
        name = 'splunk-' + str(index+1)
        tag_instance(info_dict['splunk_instance_id'], name, 'splunkchallenge')
        for intf in win_info['nets']:
            keyname = 'splunk_priv_ip#' + str(intf['intf_index'])
...
def deprov(csvfile):
    with open(csvfile, mode='r') as csv_file:
        ...
        for row in csv_reader:
            instance_list.append(row['splunk_instance_id'])
