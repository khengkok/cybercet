...
winserver2016_ami = 'win2016ad-image-v2.0'

def prov(num_instances, out_csvfile):

    # winserver2016 image 
    winserver2016ad_ami_id = get_ami_id(winserver2016_ami)
    ...
    instanceIdList = create_instances(winserver2016ad_ami_id, 
    ...
    winserver2016ad_infos = get_instances_info(instanceIdList)
    print(winserver2016ad_infos)


    # format csv
    info_dict_list = []
    
    for index, win_info in enumerate(winserver2016ad_infos):
        info_dict = {}
        info_dict['win2016ad_instance_id'] = win_info['id']
        info_dict['win2016ad_public_ip'] = win_info['public_ip']
        name = 'win2016ad-' + str(index+1)
        tag_instance(info_dict['win2016ad_instance_id'], name)
        for intf in win_info['nets']:
            keyname = 'win2016ad_priv_ip#' + str(intf['intf_index'])
...
def deprov(csvfile):
    with open(csvfile, mode='r') as csv_file:
        ...
        for row in csv_reader:
            instance_list.append(row['win2016ad_instance_id'])
